      ...
      <div className="flex flex-wrap gap-4 mb-12 border-b border-white/10 pb-4 overflow-x-auto no-scrollbar">
        {(['home', 'about', 'contact', 'footer', 'vault'] as AdminTab[]).map(tab => (
          <button
            ...
            className={`px-8 py-3 text-[10px] tracking-[0.4em] font-black uppercase transition-all ${activeTab === tab ? 'bg-brand-bone text-brand-obsidian' : 'text-brand-bone/40 hover:text-brand-bone'}`}
          >
              ...
              <section className="bg-brand-charcoal/50 p-10 border border-white/5 rounded-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="text-[10px] tracking-[0.3em] uppercase text-brand-bone/40 block mb-3 font-black italic">Fit Category</label>
                      <select value={batchConfig.category} onChange={e => setBatchConfig({...batchConfig, category: e.target.value as any})} className="w-full bg-brand-obsidian border border-white/10 p-4 outline-none uppercase tracking-widest text-xs font-bold focus:border-brand-bone/50 transition-all">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] tracking-[0.3em] uppercase text-brand-bone/40 block mb-3 font-black italic">Vault Entry Description</label>
                ...
                <div className="space-y-10">
                  ...
                  {stagedFits.length > 0 && (
                    <div className="bg-brand-obsidian p-8 border border-white/10 space-y-10 rounded-sm shadow-2xl">
                       <h4 className="text-[12px] tracking-[0.6em] font-black uppercase text-brand-bone/60 border-b border-white/10 pb-4">Staging Environment</h4>
                       <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-6">
                         {stagedFits.map(fit => (
                           <div key={fit.tempId} className="relative aspect-square bg-brand-charcoal overflow-hidden group border border-white/5">
                             <img src={fit.imageUrl} className="w-full h-full object-cover grayscale brightness-50" />
                             <button onClick={() => setStagedFits(prev => prev.filter(f => f.tempId !== fit.tempId))} className="absolute top-2 right-2 text-red-500 bg-black/80 px-2 py-1 text-[10px] font-black hover:bg-red-500 hover:text-white transition-all">REMOVE</button>
                           </div>
                         ))}
                       </div>
                       <button onClick={stampAllToVault} disabled={savingFits} className="w-full py-8 bg-brand-bone text-brand-obsidian font-black tracking-[0.8em] uppercase hover:invert transition-all text-sm">
                         {savingFits ? 'COMMITTING TO BLOCKCHAIN...' : `STAMP ${stagedFits.length} ITEMS TO VAULT`}
                       </button>
                    </div>
                  )}
                </div>
                   ...
                   {fits.map(fit => (
                     <div key={fit.id} className="relative aspect-[4/5] bg-brand-obsidian border border-white/5 group overflow-hidden">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent flex flex-col justify-end p-4 opacity-0 group-hover:opacity-100 transition-all">
                           <p className="text-[8px] tracking-widest font-black uppercase text-brand-bone mb-1">{fit.category}</p>
                           <h5 className="font-serif italic text-sm mb-3 truncate">{fit.title}</h5>
          ...
          <div className="sticky top-40 space-y-8">
            {activeTab !== 'vault' && (
              <div className="bg-brand-bone/5 border border-white/10 p-10 rounded-sm">
                ...
                <p className="text-[11px] text-brand-bone/40 mb-8 leading-relaxed font-bold">Ensure your vision is finalized before enforcing the motion. All changes hit the cloud instantly twin.</p>
                <button 
                  onClick={saveGlobalSettings} 
                  disabled={savingSettings}
                  className="w-full py-8 bg-brand-bone text-brand-obsidian font-black tracking-[0.6em] uppercase hover:invert transition-all rounded-sm shadow-2xl"
                >
            ...
            <div className="bg-brand-bone/5 border border-white/10 p-10 rounded-sm">
               <h4 className="text-[12px] tracking-[0.5em] font-black uppercase text-brand-bone/60 mb-6 border-b border-white/10 pb-4">Vision Stats</h4>
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1">
                     <p className="text-[24px] font-serif italic text-brand-bone leading-none">{fits.length}</p>
                     <p className="text-[8px] tracking-widest uppercase text-brand-bone/30 font-black">Total Fits</p>
                  </div>
                  <div className="space-y-1">
                     <p className="text-[24px] font-serif italic text-brand-bone leading-none">{fits.filter(f => f.mediaType === 'video').length}</p>
                     <p className="text-[8px] tracking-widest uppercase text-brand-bone/30 font-black">Clips</p>
                  </div>
                  <div className="space-y-1">
                     <p className="text-[24px] font-serif italic text-brand-bone leading-none">{fits.filter(f => f.section === 'gallery').length}</p>
                     <p className="text-[8px] tracking-widest uppercase text-brand-bone/30 font-black">Archive Items</p>
                  </div>
                  <div className="space-y-1">
                     <p className="text-[24px] font-serif italic text-brand-bone leading-none">V1.0</p>
                     <p className="text-[8px] tracking-widest uppercase text-brand-bone/30 font-black">Engine</p>
                  </div>
               </div>
            </div>
          </div>
